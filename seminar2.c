...
void copiazaAvioaneCuBileteReduse(struct Avion* avioane, int nrAvioane, float pragPret, struct Avion** avioaneNou, int* dimensiune)
{
	...
	for (int i = 0; i < nrAvioane; ++i)
	{
		if (avioane[i].pretBilet < pragPret) (*dimensiune)++;
	}
...
void main()
{
	...
	struct Avion* avioaneCuBileteRedue = NULL;
	int nrAvioaneCuBileteReduse = 0;
	avioane[4].pretBilet = 30;
	copiazaAvioaneCuBileteReduse(avioane, nrAvioane, 60, &avioaneCuBileteRedue, &nrAvioaneCuBileteReduse);
	printf("\n");
	afisareVectorAvioane(avioaneCuBileteRedue, nrAvioaneCuBileteReduse);
	...
	dezalocare(&avioaneCuBileteRedue, &nrAvioaneCuBileteReduse);
	free(avion.companie);
}